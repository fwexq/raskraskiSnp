...
#     return render(request, 'main/video.html', {'video': picture.objects.get(slug=slug)})
...
# class get_video(DataMixin, ListView):
#     model = picture
#     template_name = 'main/video.html'

#     def get_queryset(self, **kwargs):
#         video = picture.objects.filter(slug=self.kwargs['nazvanie_slug'])
...
#                <h3>{{el.title}}</h3>
#            {% if el.image %}
#                <p><img class="img-article-left thumb" src="{{el.image}}"></p>
#            {% endif %}
#        </div>
#    {% endfor %}
#{% endblock%}
